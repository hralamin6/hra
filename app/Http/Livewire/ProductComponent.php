...
class ProductComponent extends Component
{
    ...
    public $inputs=[];
    public $product;
    public $name, $group_id, $attribute_id, $image_link, $image,$asdf, $status='active', $quantity=0, $unit_relation=1, $category_id, $brand_id, $buying_unit_id, $selling_unit_id, $price=0, $size;
    public $selectedRows = [];
...
//        }
    ...
    {
        $this->reset('name', 'status', 'size', 'group_id', 'category_id', 'price', 'brand_id', 'quantity', 'unit_relation', 'buying_unit_id', 'selling_unit_id');
        $this->emit('openModal');
...
//            $this->product->clearMediaCollection();
           $a = $this->product->addMediaFromUrl($this->image_link)->toMediaCollection();
            unlink("media/".$a->id.'/'. $a->file_name);
        }
    ...
    {
        return Product::with('category', 'brand', 'group', 'buyingUnit', 'sellingUnit', 'invoiceDetails', 'purchaseDetails')
            ->where($this->searchBy, 'like', '%'.$this->search.'%')->orderBy($this->orderBy, $this->orderDirection)
            ->paginate($this->itemPerPage, ['id', 'selling_quantity', 'buying_quantity',
                'name', 'status', 'category_id', 'brand_id', 'quantity', 'unit_relation', 'buying_unit_id', 'selling_unit_id', 'created_at'])
            ->withQueryString();
    }
...
//        dd($this->product);
        ...
        return view('livewire.product-component', compact('items', 'categories', 'brands', 'units', 'attributes', 'groups'));
    }
}
