...
class InvoiceComponent extends Component
{
    ...
    public $name, $status='active', $quantity=0, $unit_relation=1, $category_id, $brand_id, $group_id, $buying_unit_id, $selling_unit_id;
    public $selectedRows = [];
    ...
    public $cus;
    public $cus_payment;
    public $cus_payment_details;
    public $customer_select = 'select customer';
    public $inputs = [];
    protected $queryString = [
        'customer_search_id' => ['except' => ''],
        'search' => ['except' => ''],
        'page' => ['except' => 1],
    ];
    ...
    public function mount()
    {
        ...
        if ($this->customer_search_id!=null){
            $this->cus = User::findOrFail($this->customer_search_id);
            $this->customer_select = $this->cus->name;
            $this->cus_payment = Payment::where('user_id',$this->customer_search_id)->get();
            $this->cus_payment_details = PaymentDetail::whereIn('invoice_id',$this->cus_payment->pluck('invoice_id'))->get();
    ...
    public function setCustomerValue($id, $name)
    {
        $this->customer_search_id = $id;
        $this->customer_search = '';
        $this->customer_select = $name;
        if ($this->customer_search_id!=null){
            $this->cus = User::findOrFail($id);
            $this->cus_payment = Payment::where('user_id',$id)->get();
            $this->cus_payment_details = PaymentDetail::whereIn('invoice_id',$this->cus_payment->pluck('invoice_id'))->get();
        }
    ...
    {
//        dd((float)$this->payment->due_amount);
        $data = $this->validate([
            'date' => ['required','date'],
    ...
    public function deletePayment(PaymentDetail $paymentDetail)
    {
        $this->payment->paid_amount -= ((float)$paymentDetail->current_paid_amount);
        $this->payment->due_amount += ((float)$paymentDetail->current_paid_amount);
        $this->payment->save();
...
//                $this->paymentDetail->save();
    ...
    public function changeStatus(Invoice $invoice)
    {
        ...
        foreach ($invoice->invoiceDetails as $key => $invoiceDetail) {
            ...
            if ($invoice->status=='inactive'){
                $product->quantity -= ((float)$invoiceDetail->quantity);
                $product->selling_quantity += ((float)$invoiceDetail->quantity);
                $pd->status='active';
            }else{
                $product->quantity += ((float)$invoiceDetail->quantity);
                $product->selling_quantity -= ((float)$invoiceDetail->quantity);
    ...
    {
        return Invoice::with( 'customer', 'payment')
            ->where($this->searchBy, 'like', '%'.$this->search.'%')->orderBy($this->orderBy, $this->orderDirection)
            ->when($this->customer_search_id, function ($query) {
                return $query->where('user_id', $this->customer_search_id);
                })
    ...
    public function render()
    {
        ...
        $customers = User::when($this->customer_search, function ($query) {
            return $query->where('name', 'like',  '%' . $this->customer_search . '%');
        })->where('type', 'customer')->get();
        $brands = Brand::where('status', 'active')->get();
        $groups = Group::where('status', 'active')->get();
        $units = Unit::where('status', 'active')->get();
        return view('livewire.invoice-component', compact('items', 'categories', 'brands', 'units', 'customers', 'groups'));
    }
}
