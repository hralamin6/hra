...
class PurchaseComponent extends Component
{
    ...
    public $name, $status='active', $quantity=0, $unit_relation=1, $category_id, $group_id, $brand_id, $buying_unit_id, $selling_unit_id;
    public $selectedRows = [];
    ...
    public $sup;
    public $sup_bill;
    public $sup_bill_details;
    public $supplier_select = 'select supplier';
    public $supplier_search_id;

    public $inputs = [];
    protected $queryString = [
        'supplier_search_id' => ['except' => ''],
        'search' => ['except' => ''],
        'page' => ['except' => 1],
    ];
    ...
    public function mount()
    {
        ...
        if ($this->supplier_search_id!=null){
            $this->sup = User::findOrFail($this->supplier_search_id);
            $this->sup_bill = Bill::where('user_id',$this->supplier_search_id)->get();
            $this->sup_bill_details = BillDetail::whereIn('purchase_id',$this->sup_bill->pluck('purchase_id'))->get();
            $this->supplier_select = $this->sup->name;

        }


    }
    public function setSupplierValue($id, $name)
    {
        $this->supplier_search_id = $id;
        $this->supplier_search = '';
        $this->supplier_select = $name;
        if ($this->supplier_search_id!=null){
            $this->sup = User::findOrFail($id);
            $this->sup_bill = Bill::where('user_id',$id)->get();
            $this->sup_bill_details = BillDetail::whereIn('purchase_id',$this->sup_bill->pluck('purchase_id'))->get();
            $this->supplier_select = $this->sup->name;
        }
    ...
    public function deletePayment(BillDetail $billDetail)
    {
        $this->bill->paid_amount -= ((float)$billDetail->current_paid_amount);
        $this->bill->due_amount += ((float)$billDetail->current_paid_amount);
        $this->bill->save();
        $billDetail->delete();
        $this->billDetails = BillDetail::where('purchase_id', $this->purchase->id)->get();
        $this->reset('date', 'paid_amount');
    ...
    public function add()
    {
        ...
        $this->inputs[]=(['product_id'=>$this->product_id, 'quantity'=>null, 'unit_price'=>Product::find($this->product_id)->price]);
    }
    ...
    {
        if ($k=='brand_id'|$k=='category_id'|$k=='group_id'|$k=='product_search'){
            $this->products = Product::query()
                ...
                ->when($this->group_id, function ($query) {
                    ...
                    return $query->where('name', 'like',  '%' . $this->product_search . '%');
                })
//                ->where('status', 'active')
                ->take(15)->get();
        }
    ...
    public function setProductValue($id, $name)
    {
        $this->product_id = $id;
    ...
    {
        // Focus on search input field and open dropdown when user starts typing
        $this->dispatchBrowserEvent('show-select');
    }
...
//                $this->billDetail->save();
    ...
    public function changeStatus(Purchase $purchase)
    {
        ...
        foreach ($purchase->purchaseDetails as $key => $purchaseDetail) {
            ...
            if ($purchase->status=='inactive'){
                $product->quantity += ((float)$purchaseDetail->quantity*$product->unit_relation);
                $product->buying_quantity += ((float)$purchaseDetail->quantity*$product->unit_relation);
                $pd->status='active';
            }else{
                $product->quantity -= ((float)$purchaseDetail->quantity*$product->unit_relation);
                $product->buying_quantity-= ((float)$purchaseDetail->quantity*$product->unit_relation);
    ...
    public function getDataProperty()
    {
        return Purchase::with( 'supplier')
            ...
            ->when($this->supplier_search_id, function ($query) {
                return $query->where('user_id', $this->supplier_search_id);
            })
    ...
    public function render()
    {
        ...
        $items = $this->data;
        $suppliers = User::when($this->supplier_search, function ($query) {
            return $query->where('name', 'like',  '%' . $this->supplier_search . '%');
        ...
        return view('livewire.purchase-component', compact('items', 'categories', 'brands', 'units', 'suppliers', 'groups', 'suppliers'));
    }
}
