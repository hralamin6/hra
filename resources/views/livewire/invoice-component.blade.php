...
>
                    ...
                    <aside class="border dark:border-gray-600 row-span-4 mt-4 px-4 pb-6 rounded shadow bg-white dark:bg-darkSidebar" x-data="">
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-2 mt-4 lg:grid-cols-6">
                            <div class="flex gap-2 text-sm">
                                <span class="pt-4">invoice no: #{{$invoice_no}}</span>
                        ...
                        <div class="w-full overflow-x-auto mt-8">
                            <table class="w-full whitespace-no-wrap">
                                ...
                                @foreach($inputs as $key=> $input)
                                    ...
                                    <tr id="item-id-{{$key}}" class="text-gray-700 dark:text-gray-300 capitalize">
                                        @php
                                            ...
                                            $pro = \App\Models\Product::find($inputs[$key]['product_id']);
                                        @endphp
                                        ...
                                        <td class="p-1 text-xs">
                                            ...
                                            {{ ''.$pro->quantity.'-'.$pro->buyingUnit->name.' | '.$pro->quantity*$pro->unit_relation.'-'.$pro->sellingUnit->name }}
                                        </td>
                                        <td class="p-1">
                                ...
                                <tr>
                                    ...
                                    <td class="p-1 ">
                                        <x-input wire:model.debounce="discount" class="bg-red-100 h-8" />
                        ...
                        <div x-show="!editMode" class="grid grid-cols-2 capitalize gap-4 mt-4 lg:grid-cols-4">
                            ...
                            <div>
                                ...
                                <x-input wire:model.debounce="paid_amount" />
    ...
    @include('livewire.view-invoice-details')

    <div class="flex gap-2 m-2 justify-between">
        <div class="w-24">
            <x-input wire:model.debounce.1000ms="itemPerPage" />
        </div>
        ...
        <div x-show="!editMode" x-data="{ option: false }" class="relative">
            <x-input type="text" readonly @click="option=true; $nextTick(() => $refs.input.focus())" wire:model.debounce.1000ms="customer_select" @keydown.escape="option = false" />
            <div class="absolute mt-1 w-full rounded-md bg-white dark:bg-darkBg shadow-lg z-50" x-show="option" x-on:click.away="option = false">
                <div class="py-1 overflow-y-scroll z-50 text-gray-800 dark:text-white">
                    <x-input x-ref="input"  type="text" class="px-2" wire:model.debounce.1000ms="customer_search"/>
                    <div @click="option = false" wire:click.prevent="setCustomerValue('', 'select customer')" class="px-2 py-2 dark:hover:bg-gray-900 hover:bg-gray-100 cursor-pointer">@lang('select customer')</div>
                @foreach($customers as $customer)
                        <div @click="option = false" wire:click.prevent="setCustomerValue({{$customer->id}}, '{{$customer->name}}')" class="px-2 py-2 dark:hover:bg-gray-900 hover:bg-gray-100 cursor-pointer">{{ $customer->name }}</div>
                    @endforeach
                </div>
            </div>
        </div>
...
{{--                </div>--}}
            ...
            <div class="bg-gray-100 dark:bg-gray-700 px-6 py-4 rounded-md mt-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="bg-white dark:bg-gray-800 p-4 rounded-md text-center">
                        ...
                        <p class="mt-2 text-2xl font-bold text-gray-800 dark:text-white">{{$cus_payment->sum('total_amount');}}</p>
                    </div>
                    <div class="bg-white dark:bg-gray-800 p-4 rounded-md text-center">
                        <p class="text-sm font-medium text-gray-700 dark:text-gray-300 uppercase">Total Paid</p>
                        <p class="mt-2 text-2xl font-bold text-green-600 dark:text-green-400">{{$cus_payment->sum('paid_amount');}}</p>
                    </div>
                    <div class="bg-white dark:bg-gray-800 p-4 rounded-md text-center">
                        <p class="text-sm font-medium text-gray-700 dark:text-gray-300 uppercase">Total Due</p>
                        <p class="mt-2 text-2xl font-bold text-red-600 dark:text-red-400">{{$cus_payment->sum('due_amount');}}</p>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-gray-100 dark:bg-gray-700 px-6 py-4 rounded-md">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-900 dark:text-white" id="name">{{$cus->name}}</p>
                        </div>
                        <div>
                            <label for="email" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Email Address</label>
                            <p class="mt-1 text-sm text-gray-900 dark:text-white" id="email">{{$cus->email}}</p>
                        </div>
                        <div>
                            <label for="phone" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Phone Number</label>
                            <p class="mt-1 text-sm text-gray-900 dark:text-white" id="phone">{{$cus->phone}}</p>
                        </div>
                        <div>
                            <label for="address" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Address</label>
                            <p class="mt-1 text-sm text-gray-900 dark:text-white" id="address">{{$cus->address}}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-gray-100 dark:bg-gray-700 px-6 py-4 rounded-md">
                    <div class="overflow-x-auto">
                        <table class="table-auto w-full">
                            ...
                            @foreach($cus_payment_details as $cus_payment_detail)
                                <tr>
                                    <td class="py-2 font-medium text-gray-900 dark:text-white">#{{$cus_payment_detail->invoice_id}}</td>
                                    <td class="py-2 text-gray-500 dark:text-gray-400">{{$cus_payment_detail->date}}</td>
                                    <td class="py-2 font-medium text-gray-900 dark:text-white">{{$cus_payment_detail->current_paid_amount}}</td>
                                    <td class="py-2">
    ...
    <aside class="border dark:border-gray-600 row-span-4 bg-white dark:bg-darkSidebar" x-data="{rows: @entangle('selectedRows').defer, selectPage: @entangle('selectPageRows')}">
        <div class="flex justify-between gap-3 bg-white border dark:border-gray-600 dark:bg-darkSidebar px-4 py-2">
            <p class="text-gray-600 dark:text-gray-200 capitalize">@lang('invoices')</p>
            <div class="flex justify-center capitalize gap-4 text-gray-500 dark:text-gray-300 capitalize">
                <button @click.prevent="addModal" class="flex gap-1 text-white capitalize hover:bg-blue-700 p-1 font-semibold text-sm bg-blue-500 rounded">
                    <x-h-o-plus-circle class="w-5"/>
                    @lang('add new')</button>
                <button wire:click.prevent="generate_pdf" class="flex gap-1 text-white capitalize hover:bg-blue-700 p-1 font-semibold text-sm bg-blue-500 rounded">
                        ...
                        @forelse($items as $i => $item)
                            @php
                                if ($item->payment->due_amount==0){
                                    $payment_status = 'paid';
                                }elseif($item->payment->due_amount==$item->payment->total_amount){
                                    $payment_status = 'due';
                                }else{
                                    $payment_status = __('partial');
                                }
                            @endphp
                            <tr id="item-id-{{$item->id}}" class="text-gray-700 dark:text-gray-300 capitalize" :class="{'bg-gray-200 dark:bg-gray-600': rows.includes('{{$item->id}}') }">
                                ...
                                <td class="p-1 text-sm">#{{ $item->invoice_no }} </td>
                                <td class="p-1 text-sm">{{ $item->total }} </td>
                                <td class="p-1 text-sm">{{ $item->customer->name}}-{{$item->customer->phone }} </td>
                                <td class="p-1 text-sm">{{ $item->payment->due_amount }} </td>
                                <td class="p-1 text-sm"> <span class="text-white cursor-pointer px-2 py-1 font-semibold rounded-lg
                                {{ $payment_status=='paid'?'bg-green-300 dark:bg-green-700' :($payment_status=='partial'? 'bg-blue-300 dark:bg-blue-700': ($payment_status=='due'? 'bg-red-300 dark:bg-red-700':'')) }} ">
                                    {{ $payment_status }}
                                </span></td>
