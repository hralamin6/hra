...
"
>
    <div class="flex gap-2 my-2 justify-between gap-4">
        <div class="w-24">
        ...
        <div class="w-36">
            <x-select wire:model="searchBy" class="">
                @foreach(\Illuminate\Support\Facades\Schema::getColumnListing('users') as $i=> $col)
    ...
    <aside class="border dark:border-gray-600 row-span-4 bg-white dark:bg-darkSidebar" x-data="{rows: @entangle('selectedRows').defer, selectPage: @entangle('selectPageRows')}">
        <div class="flex justify-between gap-3 bg-white border dark:border-gray-600 dark:bg-darkSidebar px-4 py-2">
            <p class="text-gray-600 dark:text-gray-200 capitalize">@lang('users')</p>
            ...
            <div class="flex justify-center capitalize gap-4 text-gray-500 dark:text-gray-300 capitalize">
                <button @click.prevent="addModal" class="flex gap-1 text-white capitalize hover:bg-blue-700 px-2 font-semibold text-sm bg-blue-500 rounded">
                ...
                <div class="w-full overflow-x-auto">
                    <table class="w-full whitespace-no-wrap">
                        ...
                        >
                            ...
                            <x-field :OB="$orderBy" :OD="$orderDirection" :field="'address'">@lang('address')</x-field>
                            <x-field :OB="$orderBy" :OD="$orderDirection" :field="'status'">@lang('status')</x-field>
                            <x-field :OB="$orderBy" :OD="$orderDirection" :field="'type'">@lang('type')</x-field>
                        ...
                        @forelse($items as $i => $item)
                            <tr id="item-id-{{$item->id}}" class="text-gray-700 dark:text-gray-300 capitalize" :class="{'bg-gray-200 dark:bg-gray-600': rows.includes('{{$item->id}}') }">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    ...
                                    <a href="{{route('purchases')}}?supplier_search_id={{$item->id}}">{{ $item->name }}</a>
                                    @elseif($item->type=='customer')
                                        <a href="{{route('invoices')}}?customer_search_id={{$item->id}}">{{ $item->name }}</a>
                                    @else
                                        {{ $item->name }}
                                    @endif
                ...
                <form @submit.prevent="editMode? $wire.editData(): $wire.saveData()" class="relative py-3 px-5 md:px-10 bg-white dark:bg-darkSidebar shadow-md rounded border border-gray-400 dark:border-gray-600 capitalize">
                    ...
                    <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <x-input wire:model.lazy="email" type="email"/>
                            @error('email')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label class="text-gray-700 dark:text-gray-200" for="address">@lang('address')</label>
                            <x-input wire:model.lazy="address" type="text"/>
                            @error('address')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label class="text-gray-700 dark:text-gray-200" for="note">@lang('note')</label>
                            <x-input wire:model.lazy="note" type="text"/>
                        ...
                        <div>
                            ...
                            <x-input wire:model.lazy="phone" type="text"/>
                        ...
                        <div>
                            ...
                            <x-select id="status" wire:model.lazy="status" >
                                <option value="admin">@lang('active')</option>
                                <option value="user">@lang('inactive')</option>
                            </x-select>
